...
import { loggerService, translationsService } from '.';
import { ConstantsGlobal } from '../core/constants';

const logger = new loggerService.Logger('/services/template.service');
...
async function renderTemplate(templateName: string, props?: unknown): Promise<string> {
    try {
        ...
        logger.error(`Unable to render template ${templateName}`, error);
        return '';
    }
}
